...
遗憾的是，为了达到这种目的，需要对程序的调用树进行全局分析。虽然全局调用树的产生方式并不烦琐，但如果程序囊括了大量的独立编译模块，全局分析的代价将会十分高昂。
当然对于这种类型的分析过程，并不是在代码开发和调试阶段完成的，'它的确可以作为一种有意义的维护期优化'。必须提醒的是，针对大型程序的这类分析将耗费大量的处理器周期
--在等待的时候可以看点书。

正如先前所讲到的, '编译器通常禁止内联复杂的方法'。这样一来，针对唯一化方法的内联例程, 其所带来的收益更多是理论上的，而非实际收益。一款先进的编译器，如果能够内联
所有的唯一化方法，将是集百利而无一害的，但当今的大部分编译器仍处在待完善阶段，'要使唯一化方法内联成为一项标准功能还任重而道远'。


// !! 精简化

精简化(trivial) 方法都是一些小型方法，通常包含 4 条以下的源代码级语句，这些语句被编译后将形成 10 条以下的汇编指令。这些方法包含的语句很少，从而产生代码膨胀的可
能性几乎为零。

'将小规模的精简化方法内联实际上将减少代码量，将较大规模的精简化方法内联可能会使总体代码量略微增加'。

总的来说，将精简化方法进行内联的最终效果不会影响代码大小。

'精简化方法经常出现在 C++ 语言中，访问类方法、间接调用以及简单的操作符重载通常为小规模方法，有时只包含一行代码，但其使用频率往往很高'。将这类方法内联，将显著地降
低因维护合理的层次化对象封装所带来的性能损失。

对于出现在编译上下文中的精简化方法, 某些编译器可将其自动内联。这表示如果精简化方法能够被编译器识别，那么它将被内联至独立的编译模块中，但对于涉及多模块的精简方法，
编译器无法将其内联。这种自动内联是一种非常便捷的功能，它可以被扩展为基于全局编译的精简化方法内联。

'为了衡量调用开销以及查看方法的指令数，以编译器为切入点是最好的方法'。

正因如此，编译器完全可以承担内联规模适中以及规模稍小方法的任务。更加高明的内联决策应当以编译器为基础，以程序员提供的各项标准为度量依据，并将代码膨胀限制在可接
受的范围内，或者对代码大小与性能的相对重要性做出合理的权衡。这种方式很容易与唯一化方法的自动内联机制相融合。

'有一点需要注意的是，内联的精简化方法需要保证其精简性不变'。如果被内联的精简化方法代码尺寸增长较大，开发人员应重新考虑这一内联决策。尤其当此方法属于非叶方法时
(叶方法不调用其他方法，在执行后返回)，更应注意这一点。代码尺寸出人意料地爆炸性增长往往是由非叶(内部)方法的内联引发的。叶方法造成的代码扩展很容易确定，首先获取
该方法的静态调用次数，之后运用其包含的指令数减去调用该方法需要的指令数，最后用所得的差乘以静态调用次数即可。当内联一个非叶方法时，由叶方法产生的所有代码扩展量都将
翻倍。
