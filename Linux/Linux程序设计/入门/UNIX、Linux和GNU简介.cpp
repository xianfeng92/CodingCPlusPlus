...
1. 什么是 UNIX
    ...
    UNIX 操作系统最初是由贝尔实验室开发的，当时的贝尔实验室是电信业巨头AT&T（美国电报电话公司）旗下的一员。UNIX 操作系统可以运
    行在大量不同种类的硬件平台上，其适用范围从 PC 工作站一直到多处理器服务器和超级计算机。

    严格来说，UNIX 是由 Open Group（开放组织）管理的一个商标，'它指的是一种遵循特定规范的计算机操作系统'。这个规范也称为单一 UNIX 
    规范(The Single UNIX Specification)，'它定义了所有必需的 UNIX 操作系统函数的名称、接口和行为'。
    ...
    1.简单性：许多很有用的 UNIX 工具是非常简单的，因此也是很小并易于理解的。“小而简单”(KISS：Keep It Small and Simple)是一种值得学习的技术。
      越大、越复杂的系统注定包含越大、越复杂的错误，而调试是我们所有人都想避免的苦差事

    2.集中性：通常,'让一个程序很好地执行一项任务要好过把所有功能都乱七八糟地堆在一起'。功能臃肿的程序难于使用和维护，'只有单一目标的程序更容易随着更好的
      算法或界面被开发出来而得到改进'。在 UNIX 中，当用户出现新的需求时，我们通常是把小工具组合起来以完成更复杂的任务，而不是试图将一个用户期望的所有功能
      放在一个大程序里

    3.可重用组件: '将应用程序的核心实现为库'。具有简单而灵活的编程接口、文档齐备的库可以帮助其他人开发出同类程序，或者把这些技术应用到新的应用领域。dbm 库
      就是一个例子，它是一组可重用的函数，而不是单一的数据库管理程序。
      
    4.过滤器：许多 UNIX 应用程序可用作过滤器。也就是说，它们对输入进行转换并产生输出。正如你将在后面看到的，UNIX 提供了一些机制，让我们可以把一些 UNIX 程序
    ...
    5.开放的文件格式：'比较成功并流行的 UNIX 程序都使用纯 ASCII 码的文本文件或 XML 文件作为配置文件和数据文件'。如果你在开发程序时采用了任一种做法，那你做
      对了！ 它使用户可以用标准工具来修改和搜索配置项，并且可以开发出新工具在数据文件上执行新的功能。ctags 源代码交叉引用系统就是一个好例子，它把符号位置信息
      以适合于搜索程序使用的正则表达式的形式记录下来。
...
Linux 能够存在并发展到今天是无数人共同努力的结果。操作系统内核本身仅仅是可用开发系统的一小部分。
Linux社区（以及其他的软件开发组织）支持自由软件的概念，即软件本身不应受限，它们应遵守 GNU（GNU 是 GNU's Not UNIX'的递归缩写）通用公共许可证（GPL）。
...
许多人认为 Linux 程序设计就是用 C 语言编程。的确，UNIX 最初是用 C 语言编写的，并且 UNIX 的大多数应用程序也是用 C 语言编写的，但 C 语言并不是 Linux 
程序员或 UNIX 程序员的唯一选择。

'我们主要集中在 C 语言上。我们将集中精力从 C 语言程序员的视角探究 Linux 编程接口'。


1.Linux 程序

    'Linux 应用程序表现为两种特殊类型的文件'： 可执行文件和脚本文件。可执行文件是计算机可以直接运行的程序，它们相当于 Windows 中的 .exe 文件。脚本文件
    是一组指令的集合，这些指令将由另一个程序（即解释器）来执行，它们相当于 Windows 中的 .bat 文件、.cmd 文件或解释执行的 BASIC 程序。
...
在 Linux 系统中，你会发现 c89、cc 和 gcc 这些命令全部或部分地指向系统的 C 语言编译器，通常是 GNU C 编译器，或 gcc。在 UNIX 系统中，C 语言编译器几乎总
被称为 cc。
...
'库是一组预先编译好的函数的集合，这些函数都是按照可重用的原则编写的'。它们通常由一组相互关联的函数组成以执行某项常见的任务，比如屏幕处理函数库
（curses和ncurses库）和数据库访问例程（dbm库）。

标准系统库文件一般存储在 /lib 和 /usr/lib 目录中。'C 语言编译器（或更确切地说是链接程序）需要知道要搜索哪些库文件，因为在默认情况下，它只搜索标准 C 语言库'。
这是从那个计算机速度还很慢而且 CPU 运行周期还很昂贵的时代遗留下来的问题。仅把库文件放在标准目录中，就希望编译器能够找到它是不够的，库文件必须遵循特定的命名规
范并且需要在命令行中明确指定。
...
函数库最简单的形式是一组处于“准备好使用”状态的目标文件。'当程序需要使用函数库中的某个函数时，它包含一个声明该函数的头文件。编译器和链接器负责将程序代码和函数
...
你可以很容易地创建和维护自己的静态库，只要使用ar(代表archive，即建立归档文件)程序和使用 gcc –c 命令对函数分别进行编译

你应该尽可能把函数分别保存到不同的源文件中。如果函数需要访问公共数据, 你可以把它们放在同一个源文件中, 并使用在该文件中声明的静态变量。
...
2. 你可以分别编译这些函数以产生要包含在库文件中的目标文件。这可以通过调用带有 -c 选项的 C 语言编译器来完成，-c 选项的作用是阻止编译器创建一个完整的
程序
...
例如，文件 /lib/libm.so 就是对实际库文件修订版本（/lib/libm.so.N，其中 N 代表主版本号）的符号链接。当 Linux 启动应用程序时，它会考虑应用程序
需要的函数库版本，以防止函数库的新版本致使旧的应用程序不能使用。

'对 Linux 系统来说，负责装载共享库并解析客户程序函数引用的程序（动态装载器）是ld.so'，也可能是 ld-linux.so.2、ld-lsb.so.2 或 ld-lsb.so.3。
...
在本例中，你看到标准 C 语言函数库(libc)是共享的（.so）, 程序需要的主版本号是 6。
