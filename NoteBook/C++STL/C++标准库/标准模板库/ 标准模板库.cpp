...
'STL 几乎不检验逻辑错误。所以逻辑问题几乎都不会引发 STL 产生异常'。事实上, C++standard 只要求两个函数必要时直接引发异常:成员函数 at() (它是下标操
作符的受验版本)以及 reserve()(如果传入的元素个数超过 max_size())。

此外, C++standard 要求, 只有寻常的(标准的)异常才可以发生, 例如因内存不足而引发的 bad_alloc, 或因客户自定义之操作行为而引发的异常。

'异常何时发生?  异常一旦发生对 STL 组件有何影响?' 

在 C++98 标准化过程中的很长时间里, 并未对此问题定义出相关的行为规范。事实上每一个异常都会引发不明确行为。如果执行某项操作的过程中抛出异常, 即使容器马
上析构, 也会导致不明确行为, 说不定程序整体崩解。因此, 如果你需要的是保证的、确定的行为, STL 无能为力, 它甚至有可能无法在异常发生时正确执行 
stack unwinding。


如何处理异常, 这是标准化过程中最晚期的几个讨论题之一。找到好的解决方法可不容易,而且花了很长时间,因为:

1. '很难确定 C++ 标准库究竟该提供怎样的安全程度'。你大概认为应该尽可能提供最佳安全性。例如你可能觉得, 对着 vector 中的任何位置插入一个新元素,要么成
   功,要么应该不产生任何效果。然而把后继元素向后移动以空出位置容纳新元素, 这种行为通常会导致异常, 而且无法复原。如果想要达成上述提出的目标, 安插动作就
   必须把 vector 的每一个元素复制到新位置去, 这对效率是莫大的折损! 如果优异效能是设计目标之一(就像 STL), 你绝对无法完美处理所有异常状况，你必须在效率
   和安全之间寻求某种妥协。

2. 毫无疑问, 面对异常的出现, "拥有保证而明确的行为且不至于遭受重大效率损失"比起"异常可能带来系统崩溃的风险", 当然较佳。然而这里存在着一种顾虑:处理异常
   的各种代码一旦出现, 很可能对效率有不利影响。这和"达成最佳效率"的设计目标互相抵触。在 C++98 标准化过程中, 编译器作者说, 原则上可以"不付出任何重大的
   效率代价"而实现出异常处理。然而事实却证明, 异常明细 exception specification 有可能引发效率损失, 所以它们在 C++11 中被 noexcept 取代了

经过种种讨论，C++标准库就"异常发生时的安全性"提供了以下基本保证: C++ 标准库面对异常时保证不发生资源泄漏, 也不会与容器的恒常特性(container invariant)
发生抵触。


考虑到强烈的需求, C++ 标准库如今做出以下保证:

1. 一般而言, 没有任何 erase()、clear()、pop_back()、pop_front()或 swap()函数会抛出异常。也没有任何被返回的迭代器的 copy 构造函数或 
   assignment 操作符会抛出异常。


2. 对于所有以节点为构造基础(node-based)的容器如 list、set、multiset、map 和 multimap, 以及 无序容器,如果节点构造失败,容器将保持不变。移除节点的
   动作保证不会失败(当然你得保证析构函数不得抛出异常)。然而,如果是对关联式容器插入多个元素,为保证已排序, 失败时无法完全恢复原状。'所有对关联式容器“插入
   单一元素”的操作, 支持 commit-or-rollback 行为'。也就是说, 要么成功,要么就不产生任何效果。此外, 所有擦除 erase 操作,无论是针对单一元素或针对多
   重元素, 肯定会成功。


3. 面对 list, 就算同时插入多个元素,  这个操作也是属于事务安全的 transaction safe。事实上, list 的所有操作, 除了 remove()、remove_if()、
   merge()、sort() 和 unique() 之外,要么成功,要么就不产生任何效果(commit-or-rollback)。至于上述各函数, C++ 标准库也提供了有条件的保证。
   所以, 如果你需要一个 transaction-safe 容器, 就用 list 吧。

4. 所有以 array 为构造基础(array-based) 的容器如 array、vector 和 deque, 安插元素时如果失败, 都不可能做到完全回滚。要达到完全回滚,就必须在安插
   动作之前复制所有（安插点之后的）后继元素。而且为了实现复制动作的完全回滚, 需要耗费大量时间。不过由于 push 和 pop 这两个操作在容器尾端执行,不需复制任
   何既有元素，所以万一发生异常，这两个操作可以保证容器恢复原状。

注意,所有这些保证都有一个前提: 析构函数不得抛出异常(C++ 中通常如此)。'C++标准库做了这个承诺,身为应用程序开发人员的你,也得做出相同承诺'。


如果你需要具备"完全 commit-or-rollback 能力"的容器, 应当使用 list (但不要调用或特别处理它的 remove()、remove_if()、merge()、sort() 和
 unique()), 或使用任何关联式容器 (但不要对它执行多重元素安插操作)。当你使用它们, 可确保不会"在任何修改动作之前"先复制元素(为求数据不遗失)--要知道,
 对一个容器而言, 复制动作极可能代价高昂。


 如果你不使用以节点为构造基础  (node-based) 的容器, 但又希望获得"完全 commit-or-rollback 能力", 只好自己动手为每一个关键操作提供一份外覆器
 (wrapper)了。
...
你可以提供自己的容器、迭代器、算法或函数对象, 前提是它们必须满足某些条件。事实上, C++ 标准库欠缺某些有用的扩充。之所以会这样是因为,在某一时间点,委员
会必须停止为它加入新性质, 把心力集中在完善既有成分上。否则工作永无休止。这就是 hash table 没有能够成为 C++98 的一部分的原因。


上述"有用的扩充"可以是迭代器、容器、算法。注意,'所有这些扩充都遵循泛型编程 (generic programming) 原则':

· 凡是行为像容器的东西，它就是个容器

· 凡是行为像迭代器的东西，它就是个迭代器

因此, 每当你有一个"像容器"的 class, 你就可以借由提供相应接口(begin()、end() 和若干类型定义等)把它整合到 STL 框架中。如果你无法为这样的 class 添
加成员, 你还是可以提供一个外覆器(wrapper class), 由它来提供相应的迭代器。

有些"像容器"的对象其实并未符合 STL 概念。例如, STL 容器必须具备一个起点和一个终点, 这就让环式容器(circular container): 如 ring buffer 之流很难
融入 STL 框架。
