...
'经过这些扫描(Scanner)、语法分析(Grammar Parser)、语义分析(Semantic Parser)、源代码优化(Source Optimizer)、代码生成和目标代码优化, 编译器忙活了这么多个步骤以
后, 源代码终于被编译成了目标代码'。

如果我们要把目标代码(Target Code) 使用汇编器(AS)编译成真正能够在机器上执行的指令, 那么 index 和 array 的地址应该从哪儿得到呢? 如果 index 和 array 定义在跟上面的
源代码同一个编译单元里面, 那么编译器可以为 index 和 array 分配空间, 确定它们的地址; 那如果是定义在其他的程序模块呢?
