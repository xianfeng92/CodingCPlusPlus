...
'这是 C++ 的一件困难事情:不太容易从程序源码看出表达式的复杂度'。
...
如果一个区段 (以 {} 括起来的区域) 或函数中有一个以上的离开点, 情况会稍微混乱一些。Destructor 必须被放在每一个离开点(当时 object 还存活)之前,
例如:
...
义了 Point object, 那就不够理想。这个道理似乎非常明显, 但许多 Pascal或C程序员使用 C++ 的时候, 仍然习惯把所有的 object 放在函数或某个区段的起始处。
...
'C++ 保证, 一定会在 main() 函数中第一次用到 identity 之前把 identity 构造出来, 而在 main() 函数结束之前把 identity 摧毁掉'。像 identity 这样的所
谓 global object 如果有 constructor 和  destructor  的话, 我们说它需要静态的初始化操作和内存释放操作。

'C++ 程序中所有的 global object 都被放置在程序的 data segment 中'。如果显式指定给它一个值, 此 object 将以该值为初值。否则 object 所配置到的内存
内容为 0。因此在下面这段代码中:
...
'编译器的策略之一就是, 无条件地在程序起始(startup) 时构造出对象来。然而这会导致所有的 local static class object 都在程序起始时被初始化', 即使它们所
在的那个函数从不曾被调用过。因此, '只在 identity() 被调用时才把 mat_identity 构造起来, 是比较好的做法 (现在的 C++Standard 已经强制要求这一点)'。
...
如果 Point 既没有定义一个 constructor 也没有定义一个 destructor, 那么我们的工作不会比建立一个"内建(build-in)类型所组成的数组"更多, 也就是说我们只要配置
足够内存以存储 10 个连续的 Point 元素即可。

然而 Point 的确定义了一个 default destructor, 所以这个 destructor 必须轮流施行于每一个元素之上。一般而言这是经由一个或多个 runtime library 函数达成的。

在 cfront 中, 我们使用一个被命名为 vec_new() 的函数, 产生出以 class object 构造而成的数组。比较新近的编译器, 包括 Borland、Microsoft 和 Sun,则是提供
两个函数, 一个用来处理"没有 virtual base class"的 class, 另一个用来处理"内含 virtual base class" 的 class。后一个函数通常被称为 vec_vnew()。
...
因此 pi 能够继续被使用, 但只能在受限制的情况下, 很像一个 void* 指针的情况。以 constructor 来配置一个 class object, 情况类似。
...
一般的 library 对于 new 运算符的实现操作都很直截了当， 但有两个精巧之处值得斟酌:
...
但语言要求每一次对 new 的调用都必须传回一个独一无二的指针。解决此问题的传统方法是传回一个指针, 指向一个默认为 1-byte 的内存区块(这就是为什么程序代码中的 
size 被设为 1 的原因)。这个实现技术的另一个有趣之处是, 它允许使用者提供一个属于自己的 _new_handler() 函数。这正是为什么每一次循环都调用 _new_handler() 
之故。
...
时, vec_new() 不会真正被调用, 因为它的主要功能是把 default constructor 施行于 class objects 所组成的数组的每一个元素身上。
...
'三种方式所获得的 c 对象, 结果都一样。其间的差异在于初始化的成本'。

一个编译器可能给我们任何保证吗? 严格地说是没有。C++Standard 允许编译器对于临时性对象的产生有完全的自由度:
