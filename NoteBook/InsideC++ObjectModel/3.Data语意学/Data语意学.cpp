...
// !! 一个 class 的 data members, 一般而言, 可以表现这个 class 在程序执行时的某种状态。Nonstatic data members 放置的是个别的 class object
// !! 感兴趣的数据, static data members 则放置的是整个 class 感兴趣的数据。

1. C++ 对象模型尽量以空间优化和存取速度优化的考虑来表现 nonstatic data members, 并且保持和 C 语言 struct 数据配置的兼容性。它把数据直接存放在每一个
   class object 之中。对于继承而来的 nonstatic data members (不管是 virtual 还是 non virtual base class)也是如此。不过并没有强制定义其间的排
   列顺序。
...
如果我问你 Point3d::x() 传回哪一个 x ? 是 class 内部那个 x, 还是外部 (extern) 那个 x  ? 今天每个人都会回答我是内部那一个。这个答案是正确的,但并不是从
过去以来一直都正确!

在 C++ 最早的编译器上, 如果在 Point3d::x() 的两个函数实例中对 x 做出取用操作, 该操作将会指向 global x object! 这样的绑定结果几乎普遍地
...
C++ Standard 以 "member scope resolution rules" 来精炼这个 "rewriting rule", 其效果是如果一个 inline 函数在 class 声明之后立刻被定义的
...
在上述例子里, 每一个 Point3d 对象是由三个 float 组成的, 顺序是 x、y、z。static data members 存放在程序的 data segment 中,和个别的 class objects 
无关。

C++ Standard 要求:在同一个 access section (也就是 private、public、protected 等区段)中, members 的排列只需符合"较晚出现的 members 在 class 
object 中有较高的地址" 这一条件即可。也就是说, 各个 members 并不一定得连续排列。什么东西可能会介于被声明的 members 之间呢 ? members 的边界调整
(alignment) 可能就需要填补一些 bytes。
...
'Static data members, 按其字面意义, 被编译器提出于 class 之外,并被视为一个 global 变量(只在 class 生命范围之内可见)'。每一个 member 的存取许
...
如果有两个 class, 每一个都声明了一个 static member freeList, 那么当它们都被放在程序的 data segment 时, 就会导致名称冲突。编译器的解决方法是暗中对
...
表面上所看到的对于 x、y、z 的直接存取, 事实上是经由一个 "implicit class object" (由 this 指针表达) 完成的。
...
是一样的。因此, 存取一个 nonstatic data member, 其效率和存取一个 C struct member 或一个 non derived class 的 member 是一样的。

现在让我们看看虚拟继承。虚拟继承将为经由 base class subobject 存取 class members 导入一层新的间接性, 比如:
...
member "时, 就会有重大的差异。这时候我们不能够说 pt 必然指向哪一种 class type (因此, 我们也就不知道编译时期这个 member 真正的 offset 位置), '这个
存取操作必须延迟至执行期, 经由一个额外的间接导引, 才能够解决'。但如果使用 origin 就不会有这些问题, 其类型无疑是 Point3d class, 而即使它继承自  virtual 
base class, members 的 offset 位置也在编译时期就固定了。

一个积极进取的编译器甚至可以静态地经由 origin 就解决掉对 x 的存取。


// !! “继承”与Data Member

'在 C++ 继承模型中, 一个 derived class object 所表现出来的东西, 是其自己的 members 加上其 base class members 的总和'。
...
这样设计的好处就是可以把管理 x 和 y 坐标的程序代码局部化。此外, 这个设计可以明显表现出两个抽象类之间的紧密关系。当这两个 classes 独立的时候, Point2d 
object 和 Point3d object 的声明和使用都不会有所改变。所以这两个抽象类的使用者不需要知道 objects 是否为独立的 classes 类型, 或是彼此之间有继承的关系。
...
Point3d object 的初始化操作或加法操作, 将需要部分的 Point2d object 和部分的 Point3d object 作为成本。'一般而言, 选择某些函数做成 inline 函数, 是设
计 class 时的一个重要课题'。
...
如果我要处理一个坐标点, 而不打算在乎它是一个 Point2d 或 Point3d 实例, 那么我需要在继承关系中提供一个 virtual function 接口。
...
1. 导入一个和 Point2d 有关的 virtual table, 用来存放它所声明的每一个 virtual functions 的地址。这个 table 的元素个数一般而言是被声明的 virtual 
   functions 的个数, 再加上一个或两个 slots (用以支持 runtime type identification)。
...
3. 加强 constructor, 使它能够为 vptr 设定初值, 让它指向 class 所对应的 virtual table。这可能意味着在 derived class 和每一个 base class 的
   constructor 中重新设定 vptr 的值。其情况视编译器优化的积极性而定。
...
虽然 class 的声明语法没有改变, 但每一件事情都不一样了: 两个 z() member functions 以及 operator+=() 运算符都成了虚拟函数; 每一个  Point3d class 
object 内含一个额外的 vptr member (继承自 Point2d); 多了一个 Point3d virtual table; 此外每一个 virtual member functions 的调用也比以前复杂了
...
然而在 iostream 的对象布局中, 我们只需要单一一份 ios subobject 就好。
...
在上述 iostream 例子中, 实现技术的挑战在于, 要找到一个足够有效的方法, 将 istream 和 ostream 各自维护的一个 ios subobject, 折叠成为一个由 iostream 
维护的单一 ios subobject, 并且还可以保存 base class 和 derived class 的指针(以及 references)之间的多态指定操作 (polymorphism assignments)。

一般的实现方法如下所述, Class 如果内含一个或多个 virtual base class subobjects, 像 istream 那样, 将被分割为两部分: 一个不变区域和一个共享区域。
不变区域中的数据, 不管后继如何衍化, 总是拥有固定的 offset(从 object 的开头算起), 所以这一部分数据可以被直接存取。至于共享区域, 所表现的就是 virtual base
class subobject。这一部分的数据, 其位置会因为每次的派生操作而有变化, 所以它们只可以被间接存取。各家编译器实现技术之间的差异就在于间接存取的方法不同。以下说明
三种主流策略。
...
1. 每一个对象必须针对其每一个 virtual base class 背负一个额外的指针。然而理想上我们却希望 class object 有固定的负担, 不因为其 virtual base classes 
   的个数而有所变化。想想看这该如何解决?

2. 由于虚拟继承串链的加长, 导致间接存取层次的增加。这里的意思是, 如果我有三层虚拟派生, 我就需要三次间接存取(经由三个 virtual base class 指针)。然而理想上
   我们却希望有固定的存取时间, 不因为虚拟派生的深度而改变。
